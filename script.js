...
const CONFIG = {
    ...
    UI: {
        GAME_OVER_FONT_SIZE: '64px',
        RESTART_FONT_SIZE: '32px',
        RESTART_OFFSET_Y: 80,
        EXIT_BLINK_ALPHA_HIGH: 1,
        EXIT_BLINK_ALPHA_LOW: 0.5,
        EXIT_BLINK_INTERVAL: 500
    },
    
    // Colores de fondo por nivel
    BACKGROUND_COLORS: [0xFFB6C1, 0xB0E0E6, 0xE6E6FA, 0xFFFACD, 0x98FB98],
...
function create() {
    ...
    // Texto de "Game Over"
    this.gameOverText = this.add.text(this.game.config.width / 2, this.game.config.height / 2, 'Game Over', { fontSize: CONFIG.UI.GAME_OVER_FONT_SIZE, fill: '#ff0000' });
    this.gameOverText.setOrigin(0.5);
    this.gameOverText.setVisible(false);

    // Instrucción para reiniciar
    this.restartText = this.add.text(this.game.config.width / 2, this.game.config.height / 2 + CONFIG.UI.RESTART_OFFSET_Y, 'Presiona R para reiniciar', { fontSize: CONFIG.UI.RESTART_FONT_SIZE, fill: '#ffffff' });
...
// Event listener para el botón de reinicio - Movido a la función create() para mejor gestión y cleanup
